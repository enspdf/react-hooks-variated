...
function DisplayCrimes({ crimes, categories }) {
  ...
  return (
    <>
      {categories.map(category => <button onClick={() => setFilterCategory(category)} key={category}>{category}</button>)}
      {filterCategory && <button onClick={() => { setFilterCategory(null) }}>reset</button>}
